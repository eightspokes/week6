...
class TaskStore: ObservableObject {
    ...
    var numberOfPersonalTasks: Binding<Int> {
        Binding<Int>(
            get: {
                let personalTasks = self.tasks.filter { $0.category == .personal }
                return personalTasks.count
            },
            set: { _ in }
        )
    }
    var numberOfWorkTasks: Binding<Int> {
        Binding<Int>(
            get: {
                let workTasks = self.tasks.filter { $0.category == .work }
                return workTasks.count
            },
            set: { _ in }
        )
    }
    var numberOfHomeTasks: Binding<Int> {
        Binding<Int>(
            get: {
                let homeTasks = self.tasks.filter { $0.category == .home }
                return homeTasks.count
            },
            set: { _ in }
        )
    }
    var numberOfNoCategoryTasks: Binding<Int> {
        Binding<Int>(
            get: {
                let noCategoryTasks = self.tasks.filter { $0.category == .noCategory }
                return noCategoryTasks.count
            },
            set: { _ in }
        )
    }
